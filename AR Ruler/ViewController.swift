...
//  AR Ruler
//
//  Created by Andrew Johnson on 7/26/23.
//
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        func calculate() {
            ...
            let distances = sqrt(pow(end.position.x - start.position.x, 2) +
                                 pow(end.position.y - start.position.y, 2) + pow(end.position.z - start.position.z, 2))
            
            
            updateText(text: "\(abs(distances) * 100 * 0.3937) inches" , atPosition: end.position)
